...
export const swapCoinsToKor = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => swapCoinsToKorSchema.parse(data))
  .handler(async ({ data }) => {
    const normalized = data.walletAddress.toLowerCase();
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        // Log transaction
        await tx.insert(transactions).values({
            id: `swp-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
            walletAddress: normalized,
            type: "convert",
        ...
        return { 
          success: true, 
          newCoins: user.coins - convertibleCoins, 
          newKor: user.doodlBalance + korToAdd 
        };
      });
      ...
      return { success: false, error: e.message || "Failed to complete transaction" };
    }
  });
